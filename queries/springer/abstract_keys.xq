...
let $header := concat('\begin{abstract}', $nl, $doc/information/abstract/text(), $nl, '\keywords{')
let $tail := concat('}', $nl, '\end{abstract}', $nl , $nl)
...
return
    if (count($keys)=1) then
       concat($header, '\keywords{', $key//text(), '}', $nl, $tail)
    else if ($key=$lastKey) then
        concat('\and ', $key//text(), $nl, $tail)
